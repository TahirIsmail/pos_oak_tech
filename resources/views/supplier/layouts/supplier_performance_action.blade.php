<div class="dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown">
        @if (check_access(array('A_DETAIL_SUPPLIER'), true))
            <a href="view_supplier_performance/{{ $supplier['slack'] }}" class="dropdown-item">{{ __("View") }}</a>
        @endif
        @if (check_access(array('A_EDIT_SUPPLIER'), true))
            <a href="edit_supplier_performance/{{ $supplier['slack'] }}" class="dropdown-item">{{ __("Edit") }}</a>
        @endif
    </div>
</div>