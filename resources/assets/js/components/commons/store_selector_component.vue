<template>
    <div class="dropdown">
        <a href="#" class="nav-link nav-link-dark dropdown-toggle text-bold" id="user_store_dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-if="selected_store.length != 0">
            <span class="pr-1" style="color: #e4e4e4">{{  $t("Choose Store") }}</span> {{ selected_store.name  | truncate(45) }}
        </a>
        <div class="dropdown-menu dropdown-menu-left store-selector-dropdown" style="z-index: 9999999999;" aria-labelledby="user_store_dropdown">
