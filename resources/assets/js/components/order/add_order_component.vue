<template>
    <div class="row m-0">
        ...
        <div class="col-md-12 p-0 bg-white border-right">
            ...
            <div class="card-header  p-0 border-bottom">
                <div class="d-flex flex-nowrap justify-content-between p-3 horizontal-scroll hide-horizontal-scroll">
                    ...
                    <button class="btn_key btn btn-light ml-3" v-on:click="get_keyboard_shortcuts()"
                        v-if="keyboard_shortcuts.length > 0"><i class="fas fa-keyboard"></i></button>

                    <div class="col-md-2 pr-0" v-if="store_restaurant_mode == true" title="Bill Type">
                        <select name="billing_type" v-model="billing_type"
                            class="form-control form-control-custom custom-select"
                            :disabled="(order_restaurant_mode == 1 && order_slack != '' && this.order_data.order.current_status.value_constant != 'CUSTOMER_ORDER')">
                    ...
                    <button class="btn btn-outline-dark ml-3 text-nowrap" v-on:click="get_digital_menu_orders_list(1)"
                        v-if="store_restaurant_mode == true"> {{ $t("Digital Menu Orders") }}</button>

                    <button class="btn btn-outline-dark ml-3 text-nowrap" v-on:click="get_running_orders_list(1)"
                        v-if="store_restaurant_mode == true"> {{ $t("Running Orders") }}</button>

                    <button class="btn btn-outline-dark ml-3 text-nowrap" v-on:click="get_hold_list"> {{ $t("Hold List")
                    }}</button>

                    <button class="btn btn-danger ml-3 text-nowrap" v-on:click="close_register"> <i
                            class="fas fa-cash-register"></i>&ensp;<span v-if="parent_register == 1">{{ $t("Close Register")
                            }}</span><span v-else>{{ $t("Exit Register") }}</span></button>
                </div>
            </div>
                ...
                <form @submit.prevent="product_info_form">
                    ...
                    <div class="form-row d-flex flex-nowrap overflow-auto">
                        <div class="col-md-2" v-for="(category_item, index) in categories" v-bind:key="index">
                            <input type="checkbox" class="check d-none" name="waiter" v-model="category"
                                v-bind:value="category_item.slack" v-bind:id="'category' + index"
                                v-on:change="load_product_with_category">
                            <label class="check-buttons w-100 text-truncate" v-bind:for="'category' + index">{{
            ...
            <div class="d-flex flex-wrap mb-5 p-3 product-list bg-light">
                ...
                <div class="col-md-8 p-0 full-height">
                                ...
                                <div class="col-12 product-card  bg-white product-grid"
                                    ...
                                    <div class="text-bold text-break overflow-hidden product-title">
                                        ...
                                        <span class="ml-1 text-primary" title="Customizable"
                                            v-if="product_list_item.customizable == 1"><i
                                                class="far fa-plus-square"></i></span>
                                        <span class="ml-1 text-primary" title="Variants Available"
                                            v-if="typeof product_list_item.variants_by_options_pos != 'undefined' && product_list_item.variants_by_options_pos != null && Object.keys(product_list_item.variants_by_options_pos).length > 0"><i
                                                class="far fa-clone"></i></span>
                                    </div>
                                    <div class="text-bold text-break overflow-hidden"
                                        v-show="product_list_item.quantity <= 10">
                                        <span class="text-warning text-caption">Only {{ product_list_item.quantity }}
                                            stock(s)
                                            left</span>
                                    </div>
                                    <div class="text-bold text-break overflow-hidden"
                                        v-show="order_restaurant_mode == 1 && product_list_item.ingredient_low_stock == true">
                                    ...
                                    <div class="text-bold text-break overflow-hidden">
                                        <span
                                            ...
                                            class="text-success text-caption">{{ $t("Discount") }} {{
                                                product_list_item.discount_code.discount_percentage }}%</span>
                                    </div>
                ...
                <div class="cart_form border-left col-md-4">
                    <div class="p-0 border-bottom">
                        <div class="card-header d-flex flex-wrap p-3 align-items-center">
                            ...
                            <button class="btn btn-outline-primary" v-on:click="show_customer_modal = true"><i
                                    class="fas fa-user-edit"></i> {{ $t("Customer") }}: {{ customer_label | truncate(26)
                                    }}</button>
                        </div>
                    </div>
                    ...
                    <div class="p-0 cart-list">
                        ...
                        <div class="d-flex flex-column pl-3 pt-3 pb-3 border-bottom" v-for="(cart_item, key, index) in cart"
                            ...
                            <div class="d-flex mb-2">
                                <span class="small text-secondary">{{ $t("Product Code") }} : {{ cart_item.product_code
                                }}</span>
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <input type="number" v-model="cart_item.quantity"
                                    v-on:input="validate_quantity(key, $event)"
                            ...
                            <div class="d-flex flex-row justify-content-between mr-2 cart-item-summary">
                                <div class="">
                                    <div class="d-flex flex-column">
                                        <div class="text-success"><i class="fas fa-tags cart-discount-tag"></i> {{
                                            $t("Discount Amount") }}: {{ cart_item.total_discount }}</div>
                                        <div class="">{{ $t("Tax Amount") }}: {{ cart_item.total_tax }}</div>
                                    </div>
                                </div>
                                <div class="">
                                    <div class="d-flex flex-column">
                                        <div class="text-right">{{ $t("Price") }}: {{ cart_item.quantity }} x {{
                                            cart_item.price }}</div>
                                    ...
                                    <div class="">
                                        <div class="d-flex flex-column">
                                            <div class="">+ {{ addon_cart_item.quantity }} x {{ addon_cart_item.name }}
                                                &#8226; <span class="text-secondary">{{ $t("Price") }}: {{
                                                    addon_cart_item.price }}</span> &#8226;
                                                <span
                    ...
                    <div
                        ...
                        <div class="d-flex justify-content-center show-more-billing-data bg-white cursor"
                            ...
                            <span class="show-more-billing-data-text">
                                <span v-if="toggle_calculation_data == 0"><i
                                        class="fas fa-angle-double-up show-more-billing-data-icon"></i> Show more</span>
                                <span v-else><i class="fas fa-angle-double-down show-more-billing-data-icon"></i> Show
                                    less</span>
                            </span>
                        </div>
                        <div class="bg" v-show="toggle_calculation_data == 1">
                            ...
                            <div class="bg d-flex justify-content-between mb-2 cart-summary-label mt-0">
                                <span class="d-inline-flex">
                                    {{ $t("Addt'l Discount") }}
                                    <input type="number" v-model="additional_discount_percentage"
                                        v-on:input="update_prices()"
                                        class="form-control form-control-sm ml-3 mr-1 additional_discount"
                                        :placeholder="$t('Discount')" min="0" max="100">%
                                </span>
                        ...
                        <div class="bg d-flex justify-content-between mb-2 cart-total">
                            <span class="red-text">{{ $t("Total") }}</span>
                            ...
                            <div v-if="order_restaurant_mode == 1">
                                    ...
                                    <div class="d-flex mt-2" v-if="billing_type == 'FINE_DINE'">
                                        <div class="flex-grow-1">
                                            <button type="submit" class="btn btn-primary btn-lg btn-block"
                                                v-shortkey="keyboard_shortcuts_formatted.SEND_TO_KITCHEN"
                                                ...
                                                v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'
                                                    v-if="processing == true"></i> {{ $t("Send to Kitchen") }}</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex mt-2" v-if="billing_type == 'QUICK_BILL'">
                                        <div class="mr-2">
                                            <button type="submit" class="btn btn-light btn-lg btn-block"
                                                v-shortkey="keyboard_shortcuts_formatted.HOLD_ORDER"
                                                @shortkey="create_order('HOLD')" @click.stop.prevent="create_order('HOLD')"
                                                v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'
                                                    v-if="processing == true"></i> {{ $t("Hold Order") }}</button>
                                        </div>
                                        <div class="flex-grow-1">
                                            <button type="submit" class="btn btn-primary btn-lg btn-block"
                                                v-shortkey="keyboard_shortcuts_formatted.CLOSE_ORDER"
                                                @shortkey="create_order('CLOSE')"
                                                @click.stop.prevent="create_order('CLOSE')"
                                                v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'
                                                    v-if="processing == true"></i> {{ $t("Close Order") }}</button>
                                        </div>
                                    </div>
                                ...
                                <div class="mt-2" v-if="order_slack != ''">
                                    ...
                                    <div class="d-flex flex-row"
                                        v-if="this.order_data.order.current_status.value_constant != 'CUSTOMER_ORDER'">
                                        <div class="mr-2"
                                            v-if="order_slack != '' && restaurant_mode_statuses.includes(this.order_data.order.current_status.value_constant)">
                                            <button type="submit" class="btn btn-light btn-lg btn-block"
                                                v-shortkey="keyboard_shortcuts_formatted.SEND_TO_KITCHEN"
                                                @shortkey="create_order('IN_KITCHEN')"
                                                @click.stop.prevent="create_order('IN_KITCHEN')"
                                                v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'
                                                    v-if="processing == true"></i> {{ $t("Update Order") }}</button>
                                        </div>
                                        <div class="mr-2">
                                            <button type="submit" class="btn btn-light btn-lg btn-block"
                                                v-shortkey="keyboard_shortcuts_formatted.HOLD_ORDER"
                                    ...
                                    <div v-if="this.order_data.order.current_status.value_constant == 'CUSTOMER_ORDER'">
                                        ...
                                        <div class="d-flex mt-2" v-if="billing_type == 'QUICK_BILL'">
                                            <div class="mr-2">
                                                <button type="submit" class="btn btn-light btn-lg btn-block"
                                                    v-shortkey="keyboard_shortcuts_formatted.HOLD_ORDER"
                                                    @shortkey="create_order('HOLD')"
                                                    @click.stop.prevent="create_order('HOLD')"
                                                    v-bind:disabled="processing == true"> <i
                                                        class='fa fa-circle-notch fa-spin' v-if="processing == true"></i> {{
                                                            $t("Hold Order") }}</button>
                                            </div>
                            ...
                            <div v-else>
                                <div class="d-flex mt-2" v-if="order_restaurant_mode == 0">
                                    ...
                                    <div class="flex-grow-1">
                                        <button type="submit" class="btn btn-primary btn-lg btn-block"
                                            v-shortkey="[keyboard_shortcuts_formatted.CLOSE_ORDER]"
                                            @shortkey="create_order('CLOSE')" @click.stop.prevent="create_order('CLOSE')"
                                            v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'
                                                v-if="processing == true"></i> {{ $t("Close Order") }}</button>
                                    </div>
                                </div>
        ...
        <modalcomponent v-show="show_customer_modal" v-on:close="show_customer_modal = false" ref='customer'>
                ...
                <div v-if="customer == null">
                    <div class="form-group">
                        ...
                        <cool-select type="text" name="filter_customer" v-model="customer_filter" class=""
                            ...
                            disable-filtering-by-search @search="load_customers($event, 'all')"
                            @select="set_customer($event)">
                            <template #item="{ item }">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="customer_name" v-model="customer_name" v-validate="'max:255'"
                            class="form-control" :placeholder="$t('Please provide Name')" autocomplete="off">
                        <span v-bind:class="{ 'error': errors.has('customer_name') }">{{ errors.first('customer_name')
                        }}</span>
                    ...
                    <div class="d-flex justify-content-center p-2">
                        <span class="text-primary text-bold cursor" v-on:click="new_customer_form()"><i
                                class="fas fa-user-plus"></i> {{ $t("Add New Customer") }}</span>
                    </div>
                ...
                <div v-else>
                    <div class="">
                        ...
                        <div class="d-flex justify-content-center mb-0">
                            ...
                            <span class="d-block">{{ (customer.phone && customer.type != 'DEFAULT') ? customer.phone : '-'
                            }}</span>
                        </div>
                        <div class="d-flex justify-content-center mb-0">
                            <span class='text-secondary'>{{ $t("Email") }}</span>&nbsp;
                            <span class="d-block">{{ (customer.email && customer.type != 'DEFAULT') ? customer.email : '-'
                            }}</span>
                        </div>
                        <div class="d-flex justify-content-center mt-4 mb-2">
                            <span class="text-danger text-bold cursor" v-on:click="dismiss_customer()"><i
                                    class="fas fa-times-circle fa-2x align-bottom"></i> Remove</span>
                        </div>
                    </div>
                </div>
            </template>
            <template v-slot:modal-footer>
                <button type="button" class="btn btn-light" v-shortkey="keyboard_shortcuts_formatted.SKIP_CUSTOMER"
                    @shortkey="select_customer('skip')" @click="select_customer('skip')"> {{ $t("Skip") }}</button>
                <button type="button" class="btn btn-primary" v-shortkey="keyboard_shortcuts_formatted.PROCEED_CUSTOMER"
                    ...
                    v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'
                        v-if="processing == true"></i> {{ $t("Proceed") }}</button>
            </template>
        </modalcomponent>
                            ...
                            <div class="d-flex flex-wrap">
                                <div class="row flex-fill">
                                    <div class="col-md-6" v-for="(business_account_item, index) in business_accounts"
                                        ...
                                        <input type="radio" class="check d-none" name="business_account"
                                            v-model="business_account" v-bind:value="business_account_item.slack"
                                        ...
                                        <label class="check-buttons w-100 text-truncate"
                                            ...
                                            :class="{ confirm_focus: index === business_account_focus }"
                                            v-shortkey="{ scroll: keyboard_shortcuts_formatted.SCROLL_BUSINESS_ACCOUNTS, choose: keyboard_shortcuts_formatted.CHOOSE_BUSINESS_ACCOUNT }"
                                            @shortkey="order_confirm_navigate($event, 'business_account', business_account_item)">{{
                                                business_account_item.label }} ({{
        business_account_item.account_type_label }})</label>
                                    </div>
                                </div>
                            </div>
                            <small id="business_account_help" class="form-text text-muted">Transaction will be saved under
                                this account</small>
                            <span v-bind:class="{ 'error': errors.has('confirmation_form.business_account') }">{{
                        ...
                        <div class="form-group">
                            ...
                            <div class="d-flex flex-wrap">
                                <div class="row flex-fill">
                                    <div class="col-md-3" v-for="(payment_method_item, index) in payment_methods"
                                        v-bind:key="index">
                                        <input type="radio" class="check d-none" name="payment_method"
                                            v-model="payment_method" v-bind:value="payment_method_item.slack"
                                        ...
                                        <label class="check-buttons w-100 text-truncate"
                                            ...
                                            v-shortkey="{ scroll: keyboard_shortcuts_formatted.SCROLL_PAYMENT_METHODS, choose: keyboard_shortcuts_formatted.CHOOSE_PAYMENT_METHOD }"
                                            @shortkey="order_confirm_navigate($event, 'payment_method', payment_method_item)"
                                            :class="{ confirm_focus: index === payment_method_focus }">{{
                                                payment_method_item.label }}</label>
                                    </div>
                                </div>
                            </div>
                            <span v-bind:class="{ 'error': errors.has('confirmation_form.payment_method') }">{{
                        ...
                        <div class="form-row mb-2" v-if="show_balance_calc_form">
                            <div class="form-group col-md-4">
                                ...
                                <input type="number" name="calc_received_amount" v-model="calc_received_amount"
                                    class="form-control form-control-lg" :placeholder="$t('Received Amount')"
                                    autocomplete="off" v-on:input="calculate_balance_amount">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="calc_order_amount">{{ $t("Order Value") }}</label>
                                <input type="number" name="calc_order_amount" v-model="calc_order_amount"
                                    class="form-control form-control-lg" :placeholder="$t('Order Value')" autocomplete="off"
                                    readonly>
                            </div>
                            <div class="form-group col-md-4">
                                ...
                                <input type="number" name="calc_balance_amount" v-model="calc_balance_amount"
                                    class="form-control form-control-lg" :placeholder="$t('Balance Amount')"
                    ...
                    <div v-if="order_restaurant_mode == 1">
                            ...
                            <div class="d-flex flex-wrap">
                                <div class="row flex-fill">
                                    <div class="col-md-3"
                                        ...
                                        v-bind:value="restaurant_order_type.order_type_constant" v-bind:key="index">
                                        ...
                                        <label class="check-buttons w-100 text-truncate"
                                            ...
                                            :class="{ confirm_focus: index === restaurant_order_type_focus }"
                                            v-shortkey="{ scroll: keyboard_shortcuts_formatted.SCROLL_ORDER_TYPES, choose: keyboard_shortcuts_formatted.CHOOSE_ORDER_TYPE }"
                                            @shortkey="order_confirm_navigate($event, 'order_type', restaurant_order_type_item)">{{
                                                restaurant_order_type_item.label }}</label>
                                    </div>
                                </div>
                            </div>
                            <span v-bind:class="{ 'error': errors.has('confirmation_form.restaurant_order_type') }">{{
                                errors.first('confirmation_form.restaurant_order_type') }}</span>
                        </div>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" name="contact_number" v-model="contact_number"
                                        ...
                                        :placeholder="$t('Contact Number')" autocomplete="off">
                                    <span v-bind:class="{ 'error': errors.has('confirmation_form.contact_number') }">{{
                                        errors.first('confirmation_form.contact_number') }}</span>
                                </div>
                                <div class="form-group col-md-8">
                                    <label for="address">{{ $t("Customer Address") }}</label>
                                    <textarea name="address" v-model="address" v-validate="'max:65535'"
                                        class="form-control textarea-small" :placeholder="$t('Address')"
                                        autocomplete="off"></textarea>
                                    <span v-bind:class="{ 'error': errors.has('confirmation_form.address') }">{{
                        ...
                        <div class="form-group col-md-12 p-0">
                            ...
                            <div class="d-flex flex-wrap">
                                <div class="row flex-fill"
                                    v-if="typeof waiters != 'undefined' && waiters != null && waiters.length > 0">
                                    <div class="col-md-3" v-for="(waiter_user, index) in waiters" v-bind:key="index">
                                        ...
                                        <label class="check-buttons w-100 text-truncate" v-bind:for="'waiter' + index">{{
                                            waiter_user.user_code }} - {{ waiter_user.fullname }}</label>
                                    </div>
                                </div>
                                <div v-else class="text-muted small">Waiters are not available! Add waiters / provide store
                                    access to waiters / choose waiter role for store.</div>
                            </div>
                        </div>

                        <div v-if="restaurant_order_type == 'DINEIN'">
                            <div class="form-group"
                                v-if="typeof vacant_tables != 'undefined' && vacant_tables != null && vacant_tables.length > 0"
                                ...
                                <div class="d-flex flex-wrap" v-if="vacant_tables.length <= 48">
                                    <div class="row flex-fill pl-3 pr-3">
                                        <div class="col-md-2 p-1 pb-0 mb-0" v-for="(vacant_table, index) in vacant_tables"
                                            ...
                                            <input type="radio" class="check d-none" name="restaurant_table"
                                                v-model="restaurant_table" v-bind:value="vacant_table.slack"
                                                v-bind:id="'vacant_table_check' + index" v-validate="'required'"
                                                key="restaurant_table">
                                            <label class="check-buttons w-100 text-truncate"
                                                v-bind:for="'vacant_table_check' + index"
                                                v-shortkey="{ scroll: keyboard_shortcuts_formatted.SCROLL_RESTAURANT_TABLES, choose: keyboard_shortcuts_formatted.CHOOSE_RESTAURANT_TABLE }"
                                                @shortkey="order_confirm_navigate($event, 'restaurant_table', vacant_table)"
                                                :class="{ confirm_focus: index === restaurant_table_focus }">{{
                                                    vacant_table.table_number }} <span class="float-right"
                                                    title="No of Occupants"><i class="fas fa-users text-muted"></i> {{
                                ...
                                <span v-bind:class="{ 'error': errors.has('confirmation_form.restaurant_table') }">{{
                                    errors.first('confirmation_form.restaurant_table') }}</span>
                            </div>
                            <div v-else class="text-muted small">Tables are not available! Add new tables / tables might be
                                occupied</div>
                        </div>
            ...
            <template v-slot:modal-footer>
                <button type="button" class="btn btn-light" v-shortkey="keyboard_shortcuts_formatted.CANCEL"
                    @shortkey="$emit('close')" @click="$emit('close')">Cancel</button>
                <button type="button" class="btn btn-primary" v-shortkey="keyboard_shortcuts_formatted.CONTINUE"
                    @shortkey="$emit('submit')" @click="$emit('submit')" v-bind:disabled="processing == true"> <i
                        class='fa fa-circle-notch fa-spin' v-if="processing == true"></i> Continue</button>
            </template>
        </modalcomponent>
                ...
                <form data-vv-scope="close_register_form" v-if="parent_register == 1">
                    <div>
                        <div class="form-group">
                            ...
                            <input type="number" name="closing_amount" v-model="closing_amount"
                                v-validate="'decimal|min_value:0'" class="form-control form-control-custom"
                                :placeholder="$t('Please provide total cash')" autocomplete="off">
                            <span v-bind:class="{ 'error': errors.has('close_register_form.closing_amount') }">{{
                        ...
                        <div class="form-group">
                            ...
                            <span v-bind:class="{ 'error': errors.has('close_register_form.credit_card_slips') }">{{
                                errors.first('close_register_form.credit_card_slips') }}</span>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="number" name="cheques" v-model="cheques" v-validate="'decimal|min_value:0'"
                                ...
                                autocomplete="off">
                            <span v-bind:class="{ 'error': errors.has('close_register_form.cheques') }">{{
            ...
            <template v-slot:modal-footer>
                <div v-if="register_amount_processing == true">
                    <span class="" v-if="register_amount_processing == true"><i class='fa fa-circle-notch fa-spin'
                            v-if="register_amount_processing == true"></i> Amount loading, please wait..</span>
                </div>
                <div v-else>
        ...
        <modalcomponent v-if="show_addon_group_modal" v-on:close="show_addon_group_modal = false"
            ...
            <template v-slot:modal-body>
                ...
                <div v-else>
                    <div class="mb-3" v-for="(choosed_addon_product_group_item, key, index) in choosed_addon_product_group"
                        v-bind:key="index">
                        <p class='text-subhead-bold mb-1'>{{ choosed_addon_product_group_item.label }} <span
                                class="small text-muted">({{ (choosed_addon_product_group_item.multiple_selection ==
                                    1) ? 'Choose Multiple' : 'Choose One' }})</span></p>
                        <div class="mb-2">
                            <div class="custom-control custom-checkbox"
                                ...
                                <span class="float-right">{{ choosed_addon_product.product['sale_amount_excluding_tax']
                                }}</span>
                            </div>
                        </div>
                    </div>
            ...
            <template v-slot:modal-body>
                ...
                <div class="d-flex flex-column" v-for="(selected_variant_list_item, option) in selected_variant_list"
                    ...
                    <div class="d-flex flex-wrap mb-1">
                        <div class="col-md-12">
                            <div class="row">
                                <div class="d-flex align-items-start p-1 mb-1 col-md-4 product"
                                    v-for="(variant_product_list_item, index) in selected_variant_list_item"
                                    v-bind:value="variant_product_list_item.product.slack" v-bind:key="index"
                                    v-on:click='add_to_cart(variant_product_list_item.product)'>
                                    <div class="col-12 p-3 bg-white product-grid h-100"
                                        v-shortkey="{ left: [keyboard_shortcuts_formatted.ARROW_LEFT], right: [keyboard_shortcuts_formatted.ARROW_RIGHT], choose: [keyboard_shortcuts_formatted.CHOOSE_PRODUCT] }"
                                        @shortkey="product_navigate($event)" :class="{ focus: index === product_focus }">
                                        <div class="d-flex flex-row-reverse"
                                            v-if="variant_product_list_item.product.images.length > 0">
                                            <img :src="(variant_product_list_item.product.images.length > 0) ? variant_product_list_item.product.images[0].thumbnail : '#'"
                                                alt=""
                                                class="rounded-circle product-image position-absolute d-none d-lg-block">
                                        </div>
                                        <div class="product-code">
                                            <span class="small text-secondary text-break">{{ $t("Product Code") }} : {{
                                                variant_product_list_item.product.product_code }}</span>
                                        </div>
                                        <div class="text-bold text-break overflow-hidden product-title">
                                            {{ variant_product_list_item.product.name | truncate(35) }}
                                            <span class="ml-1 text-primary" title="Customizable"
                                                v-if="variant_product_list_item.product.customizable == 1"><i
                                                    class="far fa-plus-square"></i></span>
                                        </div>
                                        <div class="text-bold text-break overflow-hidden"
                                            v-show="variant_product_list_item.product.quantity <= 10">
                                            <span class="text-warning text-caption">Only {{
                                                variant_product_list_item.product.quantity }} stock(s) left</span>
                                        </div>
                                        <div class="text-bold text-break overflow-hidden"
                                            v-show="order_restaurant_mode == 1 && variant_product_list_item.product.ingredient_low_stock == true">
                                            <span class="text-warning text-caption">Low on Ingredient stock</span>
                                        </div>
                                        <div class="text-bold text-break overflow-hidden">
                                            <span
                                                ...
                                                class="text-success text-caption">{{ $t("Discount") }} {{
                                                    variant_product_list_item.product.discount_code.discount_percentage
                                                }}%</span>
                                        </div>
                                        <div class="mt-auto ml-auto pt-3 text-break product-price">
                                            <span class="product-price-currency">{{ store_currency }}</span> {{
                                                variant_product_list_item.product.sale_amount_excluding_tax }}
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <quickpanelcomponent v-show="show_running_order_quickpanel"
            ...
            <template v-slot:quick-panel-body>
                ...
                <span class="" v-if="running_order_list_processing == true"><i class='fa fa-circle-notch fa-spin'
                        v-if="running_order_list_processing == true"></i> Loading..</span>
                ...
                <div v-show="running_order_list_processing == false">
                    ...
                    <div v-if="running_order_list_filtered.length > 0">
                        <div class="list-item mb-3" v-for="(running_order_list_item, index) in running_order_list_filtered"
                            v-bind:value="running_order_list_item.slack" v-bind:key="index"
                            v-on:click="go_to_order(running_order_list_item.status.constant, running_order_list_item.detail_link, running_order_list_item.edit_link)">
                            <div class="d-flex justify-content-between mb-2">
                                <div class="mr-auto">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-4 mb-0">
                                    ...
                                    <p class="mb-0">{{ (running_order_list_item.table !=
                                        "") ? running_order_list_item.table : "-" }}</p>
                                </div>
                                <div class="form-group col-md-4 mb-0">
                                    <label for="table">{{ $t("Waiter") }}</label>
                                    <p class="mb-0">{{ (running_order_list_item.waiter_data !=
                                        null) ? running_order_list_item.waiter_data.fullname + ' ('
                                        + running_order_list_item.waiter_data.user_code + ')' : "-" }}</p>
                                </div>
                            </div>
                        </div>
        ...
        <quickpanelcomponent v-show="show_hold_list_modal" v-on:close-quick-panel="show_hold_list_modal = false"
            ...
            <template v-slot:quick-panel-body>
                ...
                <span class="" v-if="hold_list_processing == true"><i class='fa fa-circle-notch fa-spin'
                        v-if="hold_list_processing == true"></i> Loading..</span>
                ...
                <div v-show="hold_list_processing == false">
                    ...
                    <div v-if="hold_list_filtered.length > 0">
                        <div class="list-item mb-3" v-for="(hold_order_list_item, key, index) in hold_list_filtered"
                            v-bind:key="index"
                            v-on:click="go_to_order(hold_order_list_item.status.constant, hold_order_list_item.detail_link, hold_order_list_item.edit_link)">
                            <div class="form-row">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6 mb-0">
                                    ...
                                    <p class="mb-0">{{ (hold_order_list_item.customer_name != "" &&
                                        hold_order_list_item.customer_name != null) ? hold_order_list_item.customer_name :
                                        "-"
                                    }}</p>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6 mb-0">
                                    ...
                                    <p class="mb-0">{{ (hold_order_list_item.customer_phone !=
                                        "") ? hold_order_list_item.customer_phone : "-" }}</p>
                                </div>
                                <div class="form-group col-md-6 mb-0">
                                    <label for="table" class="mb-0">{{ $t("Email") }}</label>
                                    <p class="mb-0 text-truncate">{{ (hold_order_list_item.customer_email !=
                                        "") ? hold_order_list_item.customer_email : "-" }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <quickpanelcomponent v-show="show_digital_menu_order_quickpanel"
            ...
            <template v-slot:quick-panel-body>
                ...
                <span class="" v-if="digital_menu_order_list_processing == true"><i class='fa fa-circle-notch fa-spin'
                        v-if="digital_menu_order_list_processing == true"></i> Loading..</span>
                ...
                <div v-show="digital_menu_order_list_processing == false">
                    ...
                    <div v-if="digital_menu_order_list_filtered.length > 0">
                        <div class="list-item mb-3"
                            v-for="(digital_menu_order_list_item, index) in digital_menu_order_list_filtered"
                            v-bind:value="digital_menu_order_list_item.slack" v-bind:key="index"
                            v-on:click="go_to_order(digital_menu_order_list_item.status.constant, digital_menu_order_list_item.detail_link, digital_menu_order_list_item.edit_link)">
                            <div class="d-flex justify-content-between mb-2">
                                <div class="mr-auto">
                                    <span class="timer-circle bg-light"><span class="timer-dot mr-1"></span> {{
                                        digital_menu_order_list_item.duration }} Minute</span>
                                    <span class="ml-2"><label for="order">Order</label> #{{
                                        digital_menu_order_list_item.order_number }}</span>
                                </div>
                                <div class="ml-auto">
                                    <span v-if="digital_menu_order_list_item.kitchen_status != null"
                                        v-bind:class="digital_menu_order_list_item.kitchen_status.color">{{
                                            digital_menu_order_list_item.kitchen_status.label }}</span>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-4 mb-0">
                                ...
                                <div class="form-group col-md-4 mb-0">
                                    <label for="table">{{ $t("Table") }}</label>
                                    <p class="mb-0">{{ (digital_menu_order_list_item.table !=
                                        "") ? digital_menu_order_list_item.table : "-" }}</p>
                                </div>
                                <div class="form-group col-md-4 mb-0">
                                    <label for="table">{{ $t("Waiter") }}</label>
                                    <p class="mb-0">{{ (digital_menu_order_list_item.waiter_data !=
                                        null) ? digital_menu_order_list_item.waiter_data.fullname + ' ('
                                        + digital_menu_order_list_item.waiter_data.user_code + ')' : "-" }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="d-flex justify-content-center pl-4 pr-4">
                            <span class="text-centered btn-label mt-2" v-show="digital_menu_order_has_more_items"
                                v-on:click="get_digital_menu_orders_list(digital_menu_order_next_page)">
