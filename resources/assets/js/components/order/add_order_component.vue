...
<template>
    ...
    <div class="row m-0">
        ...
        <div class="col-md-8 p-0 bg-white border-right">
                ...
                <form @submit.prevent="product_info_form">
                    <div class="form-row mb-2">
                        <div class="form-group col-md-3">
                            <label for="barcode">{{ $t("Product Serial No") }}</label>
                            <input type="text" name="barcode" v-model="barcode" class="form-control form-control-lg"
                                ref="barcode" :placeholder="$t('Serial No#')" autocomplete="off">
                        </div>
                        <div class="form-group col-md-3">
                            <label for="product_title">{{ $t("Product Model") }}</label>
                            <input type="text" name="product_title" v-model="product_title"
                                class="form-control form-control-lg" :placeholder="$t('Product Model')"
                                autocomplete="off">
                        </div>

                        <div class="form-group col-md-3">
                            <label for="product_title">{{ $t("Generic Search") }}</label>
                            <input type="text" name="product_title" v-model="generic_search"
                                class="form-control form-control-lg" :placeholder="$t('make, model, ram, ssd, ...')"
                                autocomplete="off">
                        </div>
                    ...
                    <div class="form-row d-flex flex-wrap">
                        <div class="col-md-2 p-1" v-for="(category_item, index) in categories" :key="index">
                            <input type="checkbox" class="check d-none" name="waiter" v-model="category"
                                :value="category_item.slack" :id="'category' + index"
                                @change="load_product_with_category">
                            <label class="check-buttons w-100 text-truncate text-center" :for="'category' + index">{{
            ...
            <div class="product-list-container bg-light">
                <div class="row">
                    <div class="product-card col-md-6 mb-4" v-for="(product_list_item, index) in product_list"
                        v-bind:value="product_list_item.product_slack" v-bind:key="index"
                        v-on:click='resolve_variants(product_list_item)'>
                        <div class="product-content bg-white p-4 rounded shadow-sm h-100"
                            ...
                            <div class="product-header d-flex justify-content-between mb-3">
                                <div class="product-image-container">
                                    <img v-if="product_list_item.images.length > 0"
                                        :src="product_list_item.images[0].thumbnail" alt=""
                                ...
                                <div class="product-meta d-flex">
                                    ...
                                    <div class="col-md-6 pr-3">
                                        <div class="product-title font-weight-bold">
                                            ...
                                            <span class="ml-1 text-primary" title="Customizable"
                                                v-if="product_list_item.customizable == 1">
                                                <i class="far fa-plus-square"></i>
                                            </span>
                                            <span class="ml-1 text-primary" title="Variants Available"
                                                v-if="typeof product_list_item.variants_by_options_pos != 'undefined' && product_list_item.variants_by_options_pos != null && Object.keys(product_list_item.variants_by_options_pos).length > 0">
                                    ...
                                    <div class="col-md-6 pl-3">
                                        <div class="product-code small text-muted">
                                            {{ $t("Serial No#") }} : {{ product_list_item.product_code }}
                                        </div>
                                        <!-- <div class="text-warning small font-weight-bold mb-2"
                                            v-if="order_restaurant_mode == 1 && product_list_item.ingredient_low_stock == true">
                                            Low on Ingredient stock
                                        </div>
                                        <div class="text-success small font-weight-bold"
                                            ...
                                            {{ $t("Discount") }} {{ product_list_item.discount_code.discount_percentage
                                            }}%
                                        </div> -->
                                    </div>
                                </div>
                            ...
                            <div class="product-body">
                                <div v-if="product_list_item.product_specifications.length > 0"
                                    class="product-specifications">
                                    <div class="spec-title mb-2">
                                        <span class="text-subhead">{{ $t("Product Specifications") }}</span>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-4 mb-2"
                                            v-for="spec in product_list_item.product_specifications" :key="spec.id">
                                            <div class="spec-label font-weight-bold">
                                                {{ spec.specification_label }}
                                            </div>
                                            <div class="spec-detail text-muted">
                                                <p v-if="spec.specification_label == 'Model'">
                                                    {{ spec.specification_details }}
                                                </p>
                                                <p v-else-if="spec.category_specification_details">
                                                    {{ spec.category_specification_details.values }}
                                                </p>
                                                <p v-else>
                                                    {{ spec.specification_details }}
                                                </p>
                            ...
                            <div class="product-footer mt-auto d-flex justify-content-between align-items-center">
                                <div class="product-price text-primary font-weight-bold">
                                    Purchase {{ product_list_item.purchase_amount_excluding_tax }}
                                </div>
                                <div class="product-price text-primary font-weight-bold">
                                    Sale {{ product_list_item.sale_amount_excluding_tax }}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="loading_more" class="loading-more">
                ...
                <form data-vv-scope="confirmation_form">
                    ...
                    <div v-if="order_status == 'CLOSE'">
                        <div class="form-group">
                            ...
                            <small id="business_account_help" class="form-text text-muted">Transaction will be saved
                                under this account</small>
                        ...
                        <div class="form-row mb-2" v-if="show_balance_calc_form">
                            <div class="form-group col-md-4">
                                ...
                                <input type="number" name="calc_order_amount" v-model="calc_order_amount"
                                    class="form-control form-control-lg" :placeholder="$t('Order Value')"
                            ...
                            <div class="form-group col-md-4">
                                <label for="calc_balance_amount"> {{ calc_balace_type }} {{ $t("Pending Amount")
                                    }}</label>
                    ...
                    <div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="cheques" v-model="cheques" v-validate="'decimal|min_value:0'"
                                ...
                                :placeholder="$t('Please provide total cheques')" autocomplete="off">
                            <span v-bind:class="{ 'error': errors.has('close_register_form.cheques') }">{{
                            errors.first('close_register_form.cheques') }}</span>
                        </div>
                    </div>
                </form>
                Are you sure you want to <span v-if="parent_register == 1">close</span><span v-else>exit</span>
                register?
            </template>
            <template v-slot:modal-footer>
                ...
                <div v-show="hold_list_processing == false">
                    ...
                    <div v-if="hold_list_filtered.length > 0">
                        <div class="list-item mb-3" v-for="(hold_order_list_item, key, index) in hold_list_filtered"
                            ...
                            null) ? hold_order_list_item.customer_name : "-" }}</p>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6 mb-0">
...
export default {
    ...
    methods: {
        product_info_form() {
            ...
            formData.append("generic_search", this.generic_search);
        ...
        load_product_with_category() {
            this.current_page = 1; // Reset page number
            this.product_list = []; // Clear current product list
            this.load_products(); // Load the first set of products
        },
        load_products() {
            ...
            formData.append("barcode", this.barcode);
            formData.append("product_title", this.product_title);
            formData.append("product_category", this.category); // Join categories by comma
            ...
            axios.post('/api/get_product', formData)
                .then((response) => {
                    ...
                    this.product_list.push(...response.data.data);
        ...
        handleScroll() {
            ...
            if (bottomOfWindow && !this.loading_more && this.current_page < this.last_page) {
                this.current_page += 1;
