                ...
                <form @submit.prevent="submit_form" class="mb-3">
                    ...
                    <div class="form-row mb-2">
                        <div class="form-group col-md-4">
                            ...
                            <cool-select type="text" name="supplier" v-validate="'required|max:250'"
                                :placeholder="$t('Please choose supplier')" autocomplete="off" v-model="supplier"
                                :items="supplier_list" item-text="label" itemValue='slack' @search='load_suppliers'
                                ref="supplier">
                            </cool-select>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" name="po_number" v-model="po_number" v-validate="'required|max:50'"
                                class="form-control form-control-custom" :placeholder="$t('Please enter PO Number')"
                                autocomplete="off">
                            <span v-bind:class="{ 'error': errors.has('po_number') }">{{ errors.first('po_number')
                            }}</span>
                        </div>
                        <div class="form-group col-md-4">
                            <label for="po_reference">{{ $t("PO Reference #") }}</label>
                            <input type="text" name="po_reference" v-model="po_reference" v-validate="'max:30'"
                                class="form-control form-control-custom" :placeholder="$t('Please enter PO Reference #')"
                                autocomplete="off">
                            <span v-bind:class="{ 'error': errors.has('po_reference') }">{{ errors.first('po_reference')
                            }}</span>
                        </div>
                    </div>
                    ...
                    <div class="form-row mb-2">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <date-picker :format="date.format" :lang='date.lang' v-model="order_date"
                                v-validate="'date_format:yyyy-MM-dd'"
                                input-class="form-control form-control-custom  bg-white" ref="order_date" name="order_date"
                                :placeholder="$t('Please enter Order Date')" autocomplete="off"></date-picker>
                            <span v-bind:class="{ 'error': errors.has('order_date') }">{{ errors.first('order_date')
                            }}</span>
                        </div>
                        <div class="form-group col-md-4">
                            <label for="order_due_date">{{ $t("PO Order Due Date") }}</label>
                            <date-picker :format="date.format" :lang='date.lang' v-model="order_due_date"
                                :disabled-date="not_before_order_date" name="order_due_date"
                                v-validate="'date_format:yyyy-MM-dd'"
                                input-class="form-control form-control-custom bg-white"
                                :placeholder="$t('Please enter Order Due Date')" autocomplete="off"></date-picker>
                            <span v-bind:class="{ 'error': errors.has('order_due_date') }">{{
                                errors.first('order_due_date') }}</span>
                        </div>
                        <div class="form-group col-md-4">
                            <label for="currency">{{ $t("Currency as per Supplier") }}</label>
                            <select name="currency" v-model="currency" v-validate="'required'"
                                class="form-control form-control-custom custom-select">
                                <option value="">Choose Currency..</option>
                                <option v-for="(currency_item, index) in currency_list"
                                    v-bind:value="currency_item.currency_code" v-bind:key="index">
                    ...
                    <div class="form-row mb-2">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select name="tax_option" v-model="tax_option" v-validate="''"
                                class="form-control form-control-custom custom-select">
                                <option value="">Choose Tax Option..</option>
                                <option v-for="(tax_option, index) in tax_options"
                            ...
                            <div class="form-group col-md-8">
                                ...
                                <textarea name="terms" v-model="terms" v-validate="'max:65535'"
                                    class="form-control form-control-custom" rows="1"
                                    :placeholder="$t('Enter Terms')"></textarea>
                    ...
                    <div class="form-row mb-2">
                        <div class="form-group col-md-6">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="update_stock_switch"
                                    v-model="update_stock">
                                <label class="custom-control-label" for="update_stock_switch">{{ $t("Update Product Stock")
                                }}</label>
                                <small class="form-text text-muted">If this option is enabled, product stock will be updated
                                    when the purchase order is "<b>closed</b>".</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-row mb-2" v-for="(product, index) in products" :key="index">
                        ...
                        <div class="form-group col-md-1">
                            <input type="number" v-bind:name="'product.discount_percentage_' + index"
                                v-model="product.discount_percentage" v-validate="'decimal|min_value:0'"
                                data-vv-as="Discount %" class="form-control form-control-custom" autocomplete="off"
                                step="0.01" min="0" v-on:input="calculate_price">
                        ...
                        <div class="form-group col-md-1">
                            <select v-bind:name="'product.tax_type_' + index" v-model="product.tax_type" v-validate="''"
                                class="form-control form-control-custom custom-select" v-on:change="calculate_price"
                                data-vv-as="Tax Type">
                                <option v-for="(tax_type, index) in tax_types" v-bind:value="tax_type.tax_type_constant"
                                    v-bind:key="index">
                                    {{ tax_type.label }}
                                </option>
                            </select>
                            <span v-bind:class="{ 'error': errors.has('product.tax_type_' + index) }">{{
                                errors.first('product.tax_type_' + index) }}</span>
                        </div>
                        <div class="form-group col-md-1">
                        ...
                        <div class="form-group col-md-1" v-if="products.length > 1">
                            <button type="button" class="btn btn-outline-danger" @click="remove_product(index)"><i
                                    class="fas fa-times"></i></button>
                        </div>
                    </div>

                    <button type="button" class="btn btn-sm btn-outline-primary" @click="add_new_product">{{ $t("Add More")
                    }}</button>
