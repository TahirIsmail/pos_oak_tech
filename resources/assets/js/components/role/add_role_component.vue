                ...
                <form @submit.prevent="submit_form" class="mb-3">
                    ...
                    <div class="form-row mb-2">
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" name="role" v-model="role" v-validate="'required|max:250'"
                                class="form-control form-control-custom" :placeholder="$t('Please enter role name')"
                                autocomplete="off">
                    ...
                    <div class="mb-2">
                        <div class="custom-control custom-checkbox mb-3" v-for="(menu, index) in menus" v-bind:key="index">
                            <input class="custom-control-input" type="checkbox" v-model="menu_selected"
                                v-bind:value="menu.menu_key" v-bind:id="menu.menu_key"
                                @click="menu_checker({ 'type': menu.type, 'current': menu.menu_key, 'parent': '', 'grand_parent': '', 'children': menu.childs, 'siblings': menu.siblings, 'event': $event })">
                            <label class="custom-control-label" v-bind:for="menu.menu_key">
                                {{ $t(menu.label) }}
                            </label>
                            <div class="mb-2">
                                <div class="custom-control custom-checkbox" v-for="(submenu_item, index) in menu.sub_menu"
                                    v-bind:key="index">
                                    <input class="custom-control-input" type="checkbox" v-model="menu_selected"
                                        v-bind:value="submenu_item.menu_key" v-bind:id="submenu_item.menu_key"
                                        @click="menu_checker({ 'type': submenu_item.type, 'current': submenu_item.menu_key, 'parent': menu.menu_key, 'grand_parent': '', 'children': submenu_item.childs, 'siblings': submenu_item.siblings, 'event': $event })">
                                    <label class="custom-control-label" v-bind:for="submenu_item.menu_key">
                                        {{ $t(submenu_item.label) }}
                                    </label>
                                    <div class="mb-2">
                                        <div class="custom-control custom-checkbox form-check-inline"
                                            v-for="(action_item, index) in submenu_item.actions" v-bind:key="index">
                                            <input class="custom-control-input" type="checkbox" v-model="menu_selected"
                                                v-bind:value="action_item.menu_key" v-bind:id="action_item.menu_key"
                                                @click="menu_checker({ 'type': action_item.type, 'current': action_item.menu_key, 'parent': submenu_item.menu_key, 'grand_parent': menu.menu_key, 'children': '', 'siblings': action_item.siblings, 'event': $event })">
