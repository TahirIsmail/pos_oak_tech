          ...
          <div class="form-group col-md-6">
            <div class="custom-control custom-switch ml-1">
              <input
                ...
                id="update_is_addon_product"
                v-model="is_addon_product"
                v-on:click="choose_product_identifier('IS_ADDON', $event)"
              />
              <label
                class="custom-control-label"
                for="update_is_addon_product"
                >{{ $t("This is an Add-on Product") }}</label
              >
              <small class="form-text text-muted">{{
                $t(
                  "If this option is enabled, product will be considered as an add-on product. Add-on products can only be tagged to a billing product via add-on groups"
                )
              }}</small>
            </div>
          </div>
          <div
            class="form-group col-md-6 border-left"
            v-if="restaurant_mode == 1"
          >
            <div class="custom-control custom-switch ml-1">
              <input
                type="checkbox"
                class="custom-control-input"
                id="update_is_ingredient"
                v-model="is_ingredient"
                v-on:click="choose_product_identifier('IS_INGREDIENT', $event)"
              />
              <label class="custom-control-label" for="update_is_ingredient">{{
                $t("This Product is an Ingredient")
              }}</label>
              <small class="form-text text-muted">{{
                $t(
                  "If this option is enabled, product will be added as an ingredient"
                )
              }}</small>
            </div>
          </div>
  ...
  methods: {
      ...
      if (ingredient_list != null && ingredient_list.length > 0) {
        ...
        for (let i = 0; i < ingredient_list.length; i++) {
          var individual_product = {
            ...
            unit_slack:
              ingredient_list[i].measurement_unit == null
                ? ""
                : ingredient_list[i].measurement_unit.slack,
          };
    ...
    update_ingredient_prices() {
      ...
      for (let i = 0; i < this.ingredients.length; i++) {
        ...
        if (ingredient_data.quantity != "") {
          ...
          if (!isNaN(quantity)) {
            var selling_price =
              parseFloat(quantity) * parseFloat(ingredient_data.sale_price);
            var purchase_price =
              parseFloat(quantity) * parseFloat(ingredient_data.purchase_price);
      ...
      this.ingredient_purchase_price =
        this.ingredient_purchase_price.toFixed(2);
      ...
      if (selected_addon_groups.length > 0) {
        for (let i = 0; i < selected_addon_groups.length; i++) {
          var add_on_values = {
            ...
            addon_group_code:
              selected_addon_groups[i].addon_group.addon_group_code,
          };
    ...
    load_variants(keywords) {
      if (typeof keywords != "undefined") {
        if (keywords.length > 0) {
          ...
          axios
            .post("/api/load_variant_products", formData)
            .then((response) => {
              if (response.data.status_code == 200) {
    ...
    remove_variant(index, product_variant_slack = "") {
      ...
      if (product_variant_slack != "") {
        ...
        axios
          .post("/api/remove_variant_product", formData)
          .then((response) => {
            if (response.data.status_code == 200) {
              this.show_response_message(response.data.msg, "Success");
            }
    ...
    check_tax_type(tax_type_item) {
      var data =
        tax_type_item.target.options[tax_type_item.target.options.selectedIndex]
          .dataset;
    ...
    calculate_sale_prices() {
      if (this.is_taxcode_inclusive == true) {
        ...
        var sale_price_excluding_tax =
          parseFloat(this.sale_price_including_tax) -
